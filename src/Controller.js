...
export default class Controller {
  ...
  update(delta) {
    ...
    // Only extend level once when player reaches a certain point
    if (!this.game.levelExtended && this.controlling.x > this.game.levelWidth / 2) {
      ...
      this.game.levelExtended = true; // Mark as extended so it doesn't happen again

      this.game.collisionMap = this.game.createCollisionMap();

      // Reposition finish line when the map extends so it stays far
      if (this.game.placeFinishLine) {
        this.game.placeFinishLine();
      }
    }
  }
}
