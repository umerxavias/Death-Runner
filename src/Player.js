...
export default class Player extends GameObject {
  ...
  attack() {
    ...
    this.game.sound && this.game.sound.playAttack();
  ...
  jump() {
    ...
    this.game.sound && this.game.sound.playJump();
    this.gravityTick = 0;
    this.velocityX = 0;
  ...
  render(delta) {
    ...
    // Skip rendering every other frame when invulnerable for flashing effect
    ...
    // Render health indicators above player
    const healthSize = 0.15;
    const healthSpacing = 0.05;
    const healthY = this.y - 0.3;
    
    for (let i = 0; i < this.maxHealth; i++) {
      const healthX = this.x + (i * (healthSize + healthSpacing));
      const color = i < this.health ? "#FF0000" : "#555555";
      this.game.render.drawRect(healthX, healthY, healthSize, healthSize, color);
    }
  ...
  update(delta) {
    ...
    if (this.invulnerable) {
      ...
      if (this.invulnerableTimer > 2) { // 2 seconds of invulnerability
        this.invulnerable = false;
        this.invulnerableTimer = 0;
      }
    }
    ...
    if (this.onGround) {
      ...
      if (!wasOnGround) {
        ...
        if (y !== this.lastLandY) {
          ...
          this.game.sound && this.game.sound.playLand();
